...
if __name__ == '__main__':
    ...
    dataset = sys.argv[2]
    split = sys.argv[3]
    partation = sys.argv[4]
    test_file = './statistics/images-list/%s_%s_%s.txt'%(dataset, split, partation)
    fid = open(test_file)
    ...
    # load the predicted results
    db_file = 'lmdb/result_%s_%s_%s_%s_att_lmdb'%(net_type, dataset, split, partation)
    ...
    dataset = pickle.load(open('./dataset/%s/dataset.pkl'%(dataset)))

    AttributeNames = dataset['att_name']
    idx_selected = dataset['selected_attribute']

    fid = open('results/%s/result_%s_%s_%s.txt'%(net_type, dataset, split, partation), 'w+')
    fid.write('label-based results of %d attributes: acc_pos; acc_neg; acc_mean; attribute name;\n'%(len(idx_selected)))
    ...
    fid.close()
    print 'all the results has been saved in results/%s/result_%s_%s_%s.txt!'%(net_type, dataset, split, partation)
